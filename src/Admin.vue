...
export default {
  ...
  computed: {
    ...
    connectedMembers() {
      return this.members.filter(m => m.parent === 'root' || this.members.some(p => p.id === m.parent));
    }
  },
  methods: {
    ...
    setRoot(id) {
      this.members.forEach(m => { if (m.parent === 'root') m.parent = null; });
      const member = this.members.find(m => m.id === id);
      if (member) member.parent = 'root';
      this.root = id;
    },
    deleteMember(id) { this.members = this.members.filter(m => m.id !== id); },
    expandAll() { this.expandedNodes = new Set(this.connectedMembers.map(m => m.id)); },
    collapseAll() { this.expandedNodes = new Set(); },
    toggleNode(id) {
...
<template>
  <div class="min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4">
    ...
    <div class="mb-6 border-b dark:border-gray-700">
      <nav class="flex space-x-8">
        <button @click="activeTab = 'tree'"
                ...
                class="py-2 px-1 border-b-2 font-medium text-sm">Tree</button>
        <button @click="activeTab = 'list'"
                :class="activeTab === 'list' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400'"
                class="py-2 px-1 border-b-2 font-medium text-sm">List</button>
        <button @click="activeTab = 'manage'"
                :class="activeTab === 'manage' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 dark:text-gray-400'"
                class="py-2 px-1 border-b-2 font-medium text-sm">Manage</button>
      </nav>
    </div>

    <!-- No root state -->
    <div v-if="!rootMember && activeTab !== 'manage'" class="text-center py-8">
      <p class="text-gray-500 dark:text-gray-400 italic">No root selected. Go to Manage tab to set a root member.</p>
    </div>
    ...
    <div v-else-if="activeTab === 'list'">
      ...
      <TreeList :members="connectedMembers" :root="rootMember" :expanded-nodes="expandedNodes" @toggle="toggleNode" />
    </div>

    <!-- Manage tab -->
    <Members v-else-if="activeTab === 'manage'" :members="members" :root="root"
             @set-root="setRoot"
             @edit="openForm"
             @add="openForm"
             @delete="deleteMember" />
